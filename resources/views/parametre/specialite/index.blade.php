...
<div class="row">
    <div class="col-xl-5">
        <div class="card card-custom">
            ...
            <form id="addForm" ng-controller="addFormCtrl" action="#">
                <input type="hidden" id="idSpecialiteToModify" value="@{{specialite.id}}" class="hidden" ng-hide="true"/>
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label>Nom de la localit&eacute; *</label>
                        <input type="text" class="form-control" name="libelle_specialite" id="libelle_specialite" ng-model="specialite.libelle_specialite" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.substr(1);" placeholder="Pédiatrie..." required/>
                    </div>
                </div>
    ...
    <div class="col-xl-7">
        <div class="card card-custom">
            <div class="card-header">
                <h3 class="card-title">
                     Liste des sp&eacute;cilit&eacute;s
                </h3>
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="table" class="table table-bordered table-hover table-checkable dataTable no-footer dtr-inline"
                                    ...
                                    data-search="true"
                                    data-toggle="table"
                                    data-url="{{url('parametre',['action'=>'liste-specialites'])}}"
                                    data-unique-id="id"
                                    data-show-toggle="false">
                                <thead>
                                    <tr role="row">
                                        <th data-field="libelle_specialite" data-searchable="true">Nom de la sp&eacute;cilit&eacute;</th>
        ...
        <form id="deleteForm" ng-controller="deleteFormCtrl" action="#">
            <div class="modal-content">
                ...
                <div class="modal-body">
                   <input type="text" id="idSpecialiteToDelete" ng-model="specialite.id" ng-hide="true" class="hidden"/>
                   @csrf
                       ...
                       <div class="d-flex align-items-center p-5">
                           ...
                           <div class="d-flex flex-column">
                               <div class="text-dark-75 text-center">
                                   ...
                                   Etes vous certains de vouloir supprimer la sp&eacute;cilit&eacute; ? <br/> <b>@{{specialite.libelle_specialite}}</b>
                               </div>
                           </div>
                       </div>
                   </div>
...
<script type="text/javascript">
    ...
    smartApp.controller('addFormCtrl', function ($scope) {
        $scope.populateForm = function (specialite) {
            $scope.specialite = specialite;
        };
    ...
    $(function () {
        ...
        $("#addForm").submit(function (e) {
            ...
            if (add==true) {
                var methode = 'POST';
                var url = "{{route('parametre.specialites.store')}}";
            }else{
                var id = $("#idSpecialiteToModify").val();
                var methode = 'PUT';
                var url = 'specialites/' + id;
             }
        ...
        $("#deleteForm").submit(function (e) {
            e.preventDefault();
            var id = $("#idSpecialiteToDelete").val();
            var formData = $(this).serialize();
            var $ajaxLoader = $(".interrogation");
            deleteAction('specialites/' + id, formData, $ajaxLoader, $table);
        });
    });

    function updateRow(idSpecialite) {
        add = false;
        var $scope = angular.element($("#addForm")).scope();
        var specialite =_.findWhere(rows, {id: idSpecialite});
        $scope.$apply(function () {
            $scope.populateForm(specialite);
        });
    }

    function deleteRow(idSpecialite) {
