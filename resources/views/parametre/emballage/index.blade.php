    ...
    <div class="col-xl-5">
        <div class="card card-custom">
            ...
            <form id="addForm" ng-controller="addFormCtrl" action="#">
                <input type="hidden" id="idEmballageToModify" value="@{{emballage.id}}" class="hidden" ng-hide="true"/>
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label>Nom de l'emballage *</label>
                        <input type="text" class="form-control" name="libelle_emballage" id="libelle_emballage" ng-model="emballage.libelle_emballage" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.substr(1);" placeholder="Blisters..." required/>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="table" class="table table-bordered table-hover table-checkable dataTable no-footer dtr-inline"
                                    ...
                                    data-search="true"
                                    data-toggle="table"
                                    data-url="{{url('parametre',['action'=>'liste-emballages'])}}"
                                    data-unique-id="id"
                                    data-show-toggle="false">
                                <thead>
                                    <tr role="row">
                                        <th data-field="libelle_emballage" data-searchable="true">Nom de l'emballage</th>
        ...
        <form id="deleteForm" ng-controller="deleteFormCtrl" action="#">
            <div class="modal-content">
                ...
                <div class="modal-body">
                   <input type="text" id="idEmballageToDelete" ng-model="emballage.id" ng-hide="true" class="hidden"/>
                   @csrf
                       ...
                       <div class="d-flex align-items-center p-5">
                           ...
                           <div class="d-flex flex-column">
                               <div class="text-dark-75 text-center"> 
                                   ...
                                   Etes vous certains de vouloir supprimer l'emballage; ? <br/> <b>@{{emballage.libelle_emballage}}</b>
                               </div>
                           </div>
                       </div>
                   </div>
...
<script type="text/javascript">
    ...
    smartApp.controller('addFormCtrl', function ($scope) {
        $scope.populateForm = function (emballage) {
            $scope.emballage = emballage;
        };
    ...
    $(function () {
        ...
        $("#addForm").submit(function (e) {
            ...
            if (add==true) {
                var methode = 'POST';
                var url = "{{route('parametre.emballages.store')}}";
            }else{
                var id = $("#idEmballageToModify").val();
                var methode = 'PUT';
                var url = 'emballages/' + id;
             }
        ...
        $("#deleteForm").submit(function (e) {
            e.preventDefault();
            var id = $("#idEmballageToDelete").val();
            var formData = $(this).serialize();
            var $ajaxLoader = $(".interrogation");
            deleteAction('emballages/' + id, formData, $ajaxLoader, $table);
        });
    });

    function updateRow(idEmballage) {
        add = false;
        var $scope = angular.element($("#addForm")).scope();
        var emballage =_.findWhere(rows, {id: idEmballage});
        $scope.$apply(function () {
            $scope.populateForm(emballage);
        });
    }
    
    function deleteRow(idEmballage) {
