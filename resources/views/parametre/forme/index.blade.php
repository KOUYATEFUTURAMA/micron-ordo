    ...
    <div class="col-xl-5">
        <div class="card card-custom">
            ...
            <form id="addForm" ng-controller="addFormCtrl" action="#">
                <input type="hidden" id="idFormeToModify" value="@{{forme.id}}" class="hidden" ng-hide="true"/>
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label>Nom de la forme *</label>
                        <input type="text" class="form-control" name="libelle_forme" id="libelle_forme" ng-model="forme.libelle_forme" onkeyup="this.value = this.value.charAt(0).toUpperCase() + this.value.substr(1);" placeholder="Comprimés..." required/>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="table" class="table table-bordered table-hover table-checkable dataTable no-footer dtr-inline"
                                    ...
                                    data-search="true"
                                    data-toggle="table"
                                    data-url="{{url('parametre',['action'=>'liste-formes'])}}"
                                    data-unique-id="id"
        ...
        <form id="deleteForm" ng-controller="deleteFormCtrl" action="#">
            <div class="modal-content">
                ...
                <div class="modal-body">
                   <input type="text" id="idFormeToDelete" ng-model="forme.id" ng-hide="true" class="hidden"/>
                   @csrf
                       ...
                       <div class="d-flex align-items-center p-5">
                           ...
                           <div class="d-flex flex-column">
                               <div class="text-dark-75 text-center"> 
                                   ...
                                   Etes vous certains de vouloir supprimer la forme ? <br/> <b>@{{forme.libelle_forme}}</b>
                               </div>
                           </div>
                       </div>
                   </div>
...
<script type="text/javascript">
    ...
    smartApp.controller('addFormCtrl', function ($scope) {
        $scope.populateForm = function (forme) {
            $scope.forme = forme;
        };
    ...
    $(function () {
        ...
        $("#addForm").submit(function (e) {
            ...
            if (add==true) {
                var methode = 'POST';
                var url = "{{route('parametre.formes.store')}}";
            }else{
                var id = $("#idFormeToModify").val();
                var methode = 'PUT';
                var url = 'formes/' + id;
             }
        ...
        $("#deleteForm").submit(function (e) {
            e.preventDefault();
            var id = $("#idFormeToDelete").val();
            var formData = $(this).serialize();
            var $ajaxLoader = $(".interrogation");
            deleteAction('formes/' + id, formData, $ajaxLoader, $table);
        });
    });

    function updateRow(idForme) {
        add = false;
        var $scope = angular.element($("#addForm")).scope();
        var forme =_.findWhere(rows, {id: idForme});
        $scope.$apply(function () {
            $scope.populateForm(forme);
        });
    }
    
    function deleteRow(idForme) {
